...
public abstract class DbRepositoryBase<TDbEntity, TId> : IDbRepository<TDbEntity, TId> where TDbEntity : class, IDbEntity<TId> where TId : IComparable<TId>, IEquatable<TId>
{
    ...
    protected DbSet<TDbEntity> DbSet { get; set; }
    ...
    /// <returns></returns>
    protected async Task<PagedResults<TDbEntity>> Filter(
    ...
    public virtual void Update(TDbEntity entity, CancellationToken cancellationToken = default)
    {
        if (entity is null)
            throw new ArgumentNullException(nameof(entity));
