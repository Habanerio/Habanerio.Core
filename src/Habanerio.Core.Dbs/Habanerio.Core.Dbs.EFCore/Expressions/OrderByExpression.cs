...
{
    private readonly new List<(Expression<Func<TDbEntity, object>> KeySelector, bool IsDescending)> _orderBys;

    public OrderByExpression()
    {
        _orderBys = new List<(Expression<Func<TDbEntity, object>> KeySelector, bool IsDescending)>();
    }
    ...
    {
        return new List<(Expression<Func<TDbEntity, object>> KeySelector, bool IsDescending)>();
    }
}